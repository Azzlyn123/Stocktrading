...
export function getScannerData(filters: {
  ...
  for (const tickerConfig of SIMULATED_TICKERS) {
    ...
    if (!state) continue;
    if (tickerConfig.ticker === "SPY" || tickerConfig.ticker === "QQQ")
      continue;
    ...
    const avgVol =
      state.bars5m.length > 0
        ? state.bars5m.reduce((s, b) => s + b.volume, 0) / state.bars5m.length
        : 1;
    const volRatio = state.dayVolume / Math.max(avgVol, 1);
    const atrRatio =
      state.atr14 > 0 ? calculateATR(state.bars5m, 14) / state.atr14 : 1;
    const qualifyingTier = selectTier(volRatio, atrRatio, TIERED_CONFIG);

    const distanceToResistancePct =
      state.resistanceLevel && state.price > 0
    ...
    if (!state.resistanceLevel)
      blockedReasons.push("No resistance level found");
    if (state.resistanceLevel && state.price < state.resistanceLevel) {
      blockedReasons.push(
        `Below resistance ($${state.resistanceLevel.toFixed(2)})`,
      );
    }
    ...
    if (!qualifyingTier)
      blockedReasons.push("No tier qualified (vol/ATR too low)");
    if (state.relStrengthVsSpy <= 0) blockedReasons.push("Weak vs SPY");
    if (state.signalState !== "IDLE")
      blockedReasons.push(`In state: ${state.signalState}`);
    ...
    items.push({
      ...
      resistanceLevel: state.resistanceLevel
        ? Number(state.resistanceLevel.toFixed(2))
        : null,
      passesFilters,
      ...
      distanceToResistancePct:
        distanceToResistancePct != null
          ? Number(distanceToResistancePct.toFixed(2))
          : null,
...
async function initializeLiveData(): Promise<boolean> {
  ...
  try {
    ...
    log(
      `Market is ${marketIsOpen ? "OPEN" : "CLOSED"}. Next open: ${clock.nextOpen}`,
      "alpaca",
    );
    ...
    const bars5mMap = await fetchMultipleHistoricalBars(
      allSymbols,
      "5Min",
      100,
    );
    ...
    const bars15mMap = await fetchMultipleHistoricalBars(
      allSymbols,
      "15Min",
      50,
    );
    ...
    for (const tickerConfig of SIMULATED_TICKERS) {
      ...
      if (snap) {
        ...
        if (snap.latestQuote) {
          ...
          state.spreadPct =
            mid > 0
              ? ((snap.latestQuote.ap - snap.latestQuote.bp) / mid) * 100
              : 0.02;
        }
      ...
      state.rvol =
        expectedVolSoFar > 0 ? state.dayVolume / expectedVolSoFar : 1.5;
    ...
    if (liveCount > 0) {
      log(
        `Loaded live data for ${liveCount}/${allSymbols.length} symbols`,
        "alpaca",
      );
      return true;
    } else {
      log(
        "No live data available from Alpaca, falling back to simulator",
        "alpaca",
      );
      return false;
    }
...
function startAlpacaStream(broadcast: (type: string, data: any) => void) {
  ...
  alpacaStream.setTradeHandler((trade: LiveTradeUpdate) => {
    ...
    const tickerConfig = SIMULATED_TICKERS.find(
      (t) => t.ticker === trade.symbol,
    );
    if (tickerConfig) {
...
function isInTradingSession(config: TieredStrategyConfig): boolean {
  ...
  for (const [start, end] of [
    config.sessions.open,
    config.sessions.mid,
    config.sessions.power,
  ]) {
...
function checkMarketCondition(
  ...
  if (direction === "LONG" && config.marketFilter.requireAboveVWAPForLong)
    return spyPrice > spyVwap;
  if (direction === "SHORT" && config.marketFilter.requireBelowVWAPForShort)
    return spyPrice < spyVwap;
  return true;
}
...
export async function startSimulatedDataFeed(
  ...
  if (!sharedUserId) {
    ...
    if (firstUser) {
      ...
      log(
        `Shared simulator initialized with user: ${firstUser.username}`,
        "simulator",
      );
    }
  }
  ...
  try {
    ...
    for (const sig of staleSignals) {
      if (sig.state && sig.state !== "CLOSED" && sig.state !== "IDLE") {
        await storage.updateSignal(sig.id, {
          state: "CLOSED",
          notes: (sig.notes ?? "") + " [Auto-closed on restart]",
          closedAt: new Date(),
        });
        log(
          `Cleaned up stale signal ${sig.ticker} (was ${sig.state})`,
          "simulator",
        );
      }
    }
  ...
  setInterval(async () => {
    const clock = await checkMarketClock().catch(() => ({
      isOpen: false,
      nextOpen: null,
      nextClose: null,
    }));
      ...
      if (tickCount % 5 === 0) {
        ...
        if (currentDataSource === "simulated") {
          ...
          if (
            ...
            state.trendPhaseBars > 3 + Math.floor(Math.random() * 3)
          ) {
            state.trendPhase = "pullback";
            state.trendPhaseBars = 0;
          } else if (
            state.trendPhase === "pullback" &&
            state.trendPhaseBars > 2 + Math.floor(Math.random() * 2)
          ) {
            state.trendPhase = Math.random() > 0.2 ? "rally" : "consolidation";
            state.trendPhaseBars = 0;
          } else if (
            state.trendPhase === "consolidation" &&
            state.trendPhaseBars > 2 + Math.floor(Math.random() * 3)
          ) {
            state.trendPhase = "rally";
            state.trendPhaseBars = 0;
          }
          ...
          const lastBar =
            state.bars5m.length > 0
              ? state.bars5m[state.bars5m.length - 1]
              : null;
          if (lastBar && state.price > 0) {
            const scaleMismatch =
              Math.abs(lastBar.close - state.price) / state.price;
          ...
          if (resistance) {
            ...
            if (resistDistPct <= 0.03) {
              ...
              state.resistanceLevel =
                state.price * (1 + 0.0005 + Math.random() * 0.001);
            }
          } else if (!state.resistanceLevel) {
            state.resistanceLevel =
              state.price * (1 + 0.0005 + Math.random() * 0.001);
          ...
          try {
            ...
            if (user) {
              ...
              if (spyCandles.length > 0 && tickerCandles.length > 0) {
                const spyMove =
                  spyCandles[spyCandles.length - 1].close / spyCandles[0].open -
                  1;
                const tickerMove =
                  tickerCandles[tickerCandles.length - 1].close /
                    tickerCandles[0].open -
                  1;
              ...
              const todayTrades = trades.filter(
                (t) =>
                  t.exitedAt &&
                  new Date(t.exitedAt).toLocaleDateString("en-US", {
                    timeZone: "America/New_York",
                  }) === todayET,
              );
              const dailyR = todayTrades.reduce(
              ...
              const sortedTodayTrades = todayTrades.sort(
                (a, b) =>
                  new Date(b.exitedAt!).getTime() -
                  new Date(a.exitedAt!).getTime(),
              );
              ...
              const passesUniverse =
                state.price >= tieredConfig.filters.minPrice &&
              ...
              if (tickCount % 15 === 0) {
                const distPct = state.resistanceLevel
                  ? (
                      ((state.resistanceLevel - state.price) / state.price) *
                      100
                    ).toFixed(2)
                  : "N/A";
                log(
                  `[${state.ticker}] state=${state.signalState} price=$${state.price.toFixed(2)} resist=$${state.resistanceLevel?.toFixed(2) ?? "none"} dist=${distPct}% session=${inSession} univ=${passesUniverse} locked=${tradingLocked} cooldown=${timeSinceLastBreakout > cooldownMs ? "clear" : "active"} src=${currentDataSource}`,
                  "scanner",
                );
              }
              ...
              if (
                ...
                state.resistanceLevel &&
                timeSinceLastBreakout > cooldownMs &&
                !hasOpenTrade &&
                !tradingLocked &&
                inSession &&
                passesUniverse
                ...
                if (currentDataSource === "live") {
                  ...
                  if (recentCandle && state.price > 0) {
                    const scaleMismatch =
                      Math.abs(recentCandle.close - state.price) / state.price;
                    if (scaleMismatch > 0.03) {
                      log(
                        `[${state.ticker}] BAR SCALE MISMATCH: barClose=$${recentCandle.close.toFixed(2)} vs livePrice=$${state.price.toFixed(2)} (${(scaleMismatch * 100).toFixed(1)}% diff). Using synthetic candle.`,
                        "scanner",
                      );
                      recentCandle = null;
                    }
                  ...
                  if (
                    recentCandle &&
                    recentCandle.close > state.resistanceLevel
                  ) {
                    shouldBreakout = true;
                    boCandle = recentCandle;
                  } else if (
                    recentCandle &&
                    recentCandle.high > state.resistanceLevel &&
                    state.price > state.resistanceLevel
                  ) {
                    shouldBreakout = true;
                    boCandle = recentCandle;
                  ...
                  const nearResistance =
                    state.price >= state.resistanceLevel * 0.995;
                  shouldBreakout =
                    nearResistance ||
                    (state.trendPhase === "rally" && Math.random() > 0.4);
                  boCandle = createBreakoutCandle(state, state.resistanceLevel);
                }

                if (shouldBreakout) {
                  log(
                    `[${state.ticker}] BREAKOUT CANDIDATE: price=$${state.price.toFixed(2)} resist=$${state.resistanceLevel.toFixed(2)} candleClose=$${boCandle.close.toFixed(2)} candleHigh=$${boCandle.high.toFixed(2)}`,
                    "scanner",
                  );
                  ...
                  if (usingSyntheticCandle) {
                    ...
                    const avgVol =
                      state.bars5m.length > tieredConfig.strategy.volumeLookback
                        ? state.bars5m
                            .slice(-tieredConfig.strategy.volumeLookback)
                            .reduce((s, b) => s + b.volume, 0) /
                          tieredConfig.strategy.volumeLookback
                        : state.bars5m.reduce((s, b) => s + b.volume, 0) /
                          Math.max(state.bars5m.length, 1);
                    volRatio = boCandle.volume / Math.max(avgVol, 1);
                  }

                  const rawAtr = calculateATR(
                    state.bars5m,
                    tieredConfig.strategy.atrLen,
                  );
                  ...
                  if (tier && consecutiveLosses >= 2) {
                    const demoted =
                      tier === "A" ? "B" : tier === "B" ? "C" : null;
                    if (demoted) {
                      log(
                        `[${state.ticker}] Tier demoted ${tier}→${demoted} after ${consecutiveLosses} consecutive losses`,
                        "scanner",
                      );
                      tier = demoted;
                    } else {
                      log(
                        `[${state.ticker}] Tier C skipped after ${consecutiveLosses} consecutive losses`,
                        "scanner",
                      );
                      tier = null;
                    }
                  ...
                  log(
                    `[${state.ticker}] Tier selection: volRatio=${volRatio.toFixed(2)} atrRatio=${atrRatio.toFixed(2)} tier=${tier ?? "NONE"} consLosses=${consecutiveLosses}`,
                    "scanner",
                  );

                  if (tier) {
                    const marketOk = checkMarketCondition(
                      spyBars5m,
                      tieredConfig,
                      tier,
                      direction,
                    ...
                    log(
                      `[${state.ticker}] Market check: marketOk=${marketOk} tier=${tier}`,
                      "scanner",
                    );
                    if (marketOk) {
                      const breakoutResult = checkTieredBreakout(
                        ...
                        usingSyntheticCandle ? volRatio : undefined,
                      );
                      log(
                        `[${state.ticker}] Breakout qualification: qualified=${breakoutResult.qualified} reasons=${breakoutResult.reasons.join("; ") || "all passed"}`,
                        "scanner",
                      );
                      ...
                      if (breakoutResult.qualified) {
                        ...
                        await storage.createSignal({
                          ...
                          resistanceLevel: Number(
                            state.resistanceLevel.toFixed(2),
                          ),
                          ...
                          rvol: Number(state.rvol.toFixed(2)),
                          atrValue: Number(
                            (state.atr14 * (state.scaleFactor || 1)).toFixed(4),
                          ),
                          rejectionCount: 2,
                          score: state.lastScore,
                          scoreTier: tier,
              ...
              if (
                ...
                if (
                  state.retestBarsSinceBreakout >
                  tierConfig.retestTimeoutCandles
                ) {
                  log(
                    `[${state.ticker}] BREAKOUT timed out after ${state.retestBarsSinceBreakout} candles (max ${tierConfig.retestTimeoutCandles}). Resetting to IDLE.`,
                    "scanner",
                  );
                  const staleBreakoutSignals = await storage.getAllSignals();
                  const stale = staleBreakoutSignals.find(
                    (s) => s.ticker === state.ticker && s.state === "BREAKOUT",
                  );
                  if (stale) {
                    await storage.updateSignal(stale.id, {
                      state: "CLOSED",
                      notes: (stale.notes ?? "") + " [Retest timeout]",
                      closedAt: new Date(),
                    });
                  }
                  state.signalState = "IDLE";
                  ...
                  log(
                    `[${state.ticker}] BREAKOUT→RETEST check (bar ${state.retestBarsSinceBreakout}/${tierConfig.retestTimeoutCandles}): valid=${retestResult.valid} reasons=${retestResult.reasons.join("; ") || "all passed"} candleClose=$${retCandle.close.toFixed(2)} level=$${state.resistanceLevel.toFixed(2)}`,
                    "scanner",
                  );
                  ...
                  if (retestResult.valid) {
                    ...
                    log(
                      `[${state.ticker}] >> RETEST CONFIRMED. Tier ${state.selectedTier}. Waiting for entry.`,
                      "scanner",
                    );
                    ...
                    const activeSignal = existingSignals.find(
                      (s) =>
                        s.ticker === state.ticker && s.state === "BREAKOUT",
                    );
                    if (activeSignal) {
                      await storage.updateSignal(activeSignal.id, {
                        state: "RETEST",
                        currentPrice: Number(state.price.toFixed(2)),
                      });
                    }
              ...
              if (
                ...
                if (
                  ...
                  log(
                    `[${state.ticker}] RETEST timed out after ${state.retestBarsSinceBreakout} candles. Resetting to IDLE.`,
                    "scanner",
                  );
                  const staleRetestSignals = await storage.getAllSignals();
                  const staleRetest = staleRetestSignals.find(
                    (s) =>
                      s.ticker === state.ticker &&
                      (s.state === "RETEST" || s.state === "BREAKOUT"),
                  );
                  if (staleRetest) {
                    await storage.updateSignal(staleRetest.id, {
                      state: "CLOSED",
                      notes: (staleRetest.notes ?? "") + " [Retest timeout]",
                      closedAt: new Date(),
                    });
                  }
                  state.signalState = "IDLE";
                  ...
                  log(
                    `[${state.ticker}] RETEST→ENTRY check: valid=${retestResult.valid} entry=$${retestResult.entryPrice?.toFixed(2) ?? "none"} stop=$${retestResult.stopPrice?.toFixed(2) ?? "none"} reasons=${retestResult.reasons.join("; ") || "all passed"} session=${inSession} locked=${tradingLocked}`,
                    "scanner",
                  );
                  ...
                  if (
                    ...
                    if (!emaAligned) {
                      log(
                        `[${state.ticker}] ENTRY BLOCKED: EMA9 ($${ema9.toFixed(2)}) <= EMA20 ($${ema20.toFixed(2)})`,
                        "scanner",
                      );
                    }

                    const riskPerShare = Math.abs(
                      retestResult.entryPrice - retestResult.stopPrice,
                    );
                    const nextResistance = state.resistanceLevel
                      ? state.resistanceLevel * 1.01
                      : retestResult.entryPrice * 1.02;
                    const roomTo2R =
                      nextResistance - retestResult.entryPrice >=
                      riskPerShare * 2;
                    if (!roomTo2R) {
                      log(
                        `[${state.ticker}] ENTRY BLOCKED: No 2R room. Room=$${(nextResistance - retestResult.entryPrice).toFixed(2)} vs 2R=$${(riskPerShare * 2).toFixed(2)}`,
                        "scanner",
                      );
                    }
                    ...
                    if (!emaAligned || !roomTo2R) {
                      log(
                        `[${state.ticker}] Entry gates failed. EMA=${emaAligned} Room2R=${roomTo2R}. Skipping entry.`,
                        "scanner",
                      );
                    }
                    ...
                    try {
                      ...
                      if (learningPenalty > 0) {
                        log(
                          `[${state.ticker}] LEARNING PENALTY: -${learningPenalty} points. ${learningReasons.join("; ")}`,
                          "scanner",
                        );
                      }
                    } catch (lpErr) {
                      log(
                        `[${state.ticker}] Learning penalty error: ${lpErr}`,
                        "scanner",
                      );
                    }

                    const adjustedScore =
                      (state.lastScore ?? 0) - learningPenalty;
                    const scorePassesAfterPenalty =
                      adjustedScore >=
                      (tieredConfig.daily.maxDailyLossR !== undefined
                        ? 35
                        : 35);
                    if (!scorePassesAfterPenalty && learningPenalty > 0) {
                      log(
                        `[${state.ticker}] ENTRY BLOCKED by learning: score ${state.lastScore} - ${learningPenalty} penalty = ${adjustedScore} (below 30 min)`,
                        "scanner",
                      );
                    }
                    ...
                    if (emaAligned && roomTo2R && scorePassesAfterPenalty) {
                      ...
                      const maxPosSize =
                        equity * (tieredConfig.risk.maxPositionPct / 100);
                      if (shares * retestResult.entryPrice > maxPosSize)
                        shares = Math.floor(
                          maxPosSize / retestResult.entryPrice,
                        );

                      if (shares > 0) {
                        log(`[ENTRY GATE] emaAligned=${emaAligned} roomTo2R=${roomTo2R} adjustedScore=${adjustedScore} passes=${scorePassesAfterPenalty} learningPenalty=${learningPenalty}`, "scanner");
                        state.signalState = "TRIGGERED";
                        const target1 =
                          retestResult.entryPrice +
                          riskPerShare * tieredConfig.exits.partialAtR;
                        const target2 =
                          retestResult.entryPrice +
                          riskPerShare * tieredConfig.exits.finalTargetR;
                        log(
                          `[${state.ticker}] >> TRADE TRIGGERED! Tier ${state.selectedTier} entry=$${retestResult.entryPrice.toFixed(2)} stop=$${retestResult.stopPrice.toFixed(2)} shares=${shares} risk=$${riskDollars.toFixed(0)} T1=$${target1.toFixed(2)} T2=$${target2.toFixed(2)}`,
                          "scanner",
                        );

                        const existingSignals2 = await storage.getAllSignals();
                        const activeSignal2 = existingSignals2.find(
                          (s) =>
                            s.ticker === state.ticker &&
                            (s.state === "RETEST" || s.state === "BREAKOUT"),
                        );
                        if (activeSignal2) {
                          await storage.updateSignal(activeSignal2.id, {
                            ...
                            riskReward:
                              riskPerShare > 0
                                ? (target2 - retestResult.entryPrice) /
                                  riskPerShare
                                : 0,
                        ...
                        const trade = await storage.createTrade({
                          ...
                          entryPrice: retestResult.entryPrice,
                          stopPrice: retestResult.stopPrice,
                          originalStopPrice: retestResult.stopPrice,
                          target1,
                          target2,
                          shares,
                          status: "open",
              ...
              for (const trade of openTrades) {
                const minutesSinceEntry = Math.floor(
                  (Date.now() - new Date(trade.enteredAt!).getTime()) / 60000,
                );
                const riskPerShare = Math.abs(
                ...
                if (exitDecision.shouldExit) {
                  if (exitDecision.exitType === "partial") {
                    ...
                    if (priceDiff > 0.2) {
                      log(
                        `[${state.ticker}] EXIT SANITY FAIL: exitPrice=$${exitPrice.toFixed(2)} vs entry=$${trade.entryPrice.toFixed(2)} (${(priceDiff * 100).toFixed(1)}% diff). Using entry as exit.`,
                        "scanner",
                      );
                      const sanitizedExit = trade.entryPrice;
                      const pnl =
                        (sanitizedExit - trade.entryPrice) * trade.shares;
                      const realizedR =
                        riskPerShare > 0
                          ? (sanitizedExit - trade.entryPrice) / riskPerShare
                          : 0;
                      await storage.updateTrade(trade.id, {
                        status: "closed",
                        exitPrice: sanitizedExit,
                        exitedAt: new Date(),
                        exitReason: "sanity_check",
                        pnl,
                        realizedR,
                        pnlPercent: 0,
                      });
