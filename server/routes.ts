...
import {
  ...
  runORFSimulation,
  runRSContinuationSimulation,
  runGapContinuationSimulation,
  runSmallCapMomentumSimulation,
...
import {
  SMALLCAP_SCAN_TICKERS,
  buildScanDatesRange,
} from "./strategy/smallCapUniverse";
...
export async function registerRoutes(
  ...
  // Quick access — auto-creates the admin account if it doesn't exist, then logs in
  app.post("/api/quick-access", async (req, res, next) => {
    try {
      const username = "admin";
      const password = "breakoutiq";
  ...
  const STRATEGY_RULE_FIELDS = new Set([
    "breakoutMinBodyPct",
    "breakoutMinRangeMultiplier",
    "retestMaxPullbackPct",
    "entryMode",
    "maxVwapCrosses",
    "chopSizeReduction",
    "volGateFirstRangePct",
    "volGateAtrMultiplier",
    "scoreFullSizeMin",
    "scoreHalfSizeMin",
    "riskMode",
    "powerSetupEnabled",
  ...
  app.patch("/api/settings", requireAuth, async (req, res, next) => {
    try {
      ...
      const hasRuleChange = incomingKeys.some((k) =>
        STRATEGY_RULE_FIELDS.has(k),
      );
  ...
  app.post("/api/simulations", requireAuth, async (req, res) => {
    ...
    if (!simulationDate || typeof simulationDate !== "string") {
      return res
        .status(400)
        .json({ error: "simulationDate is required (YYYY-MM-DD)" });
    }
    ...
    if (simDate >= today) {
      return res
        .status(400)
        .json({ error: "Simulation date must be in the past." });
    }
  ...
  app.post("/api/simulations/:id/cancel", requireAuth, async (req, res) => {
    ...
    if (!cancelled)
      return res
        .status(404)
        .json({ error: "Simulation not found or already completed" });
    res.json({ success: true });
  });

  app.post("/api/simulations/auto-run", requireAuth, async (req, res) => {
    const userId = (req.user as User).id;
    const { durationMinutes, exactDays, disableLearning, startDate, endDate } =
      req.body;
    // When a date range or large exactDays is given, allow up to 24 hours; otherwise cap at 60 min
    const hasLargeBatch =
      (exactDays && Number(exactDays) > 10) || (startDate && endDate);
    const duration = hasLargeBatch
      ? Math.min(Math.max(Number(durationMinutes) || 1440, 1), 1440)
      : Math.min(Math.max(Number(durationMinutes) || 5, 1), 60);
    const days = exactDays
      ? Math.min(Math.max(Number(exactDays), 1), 500)
      : undefined;
  ...
  app.post("/api/simulations/auto-run/cancel", requireAuth, (_req, res) => {
    ...
    if (!cancelled)
      return res.status(404).json({ error: "No active auto-run to cancel" });
    res.json({ success: true });
  });

  app.post(
    "/api/simulations/:id/cost-sensitivity",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as User;
        const simulationId = Array.isArray(req.params.id)
          ? req.params.id[0]
          : req.params.id;
        ...
        res
          .status(500)
          .json({ error: err.message || "Cost sensitivity analysis failed" });
      }
    },
  );
  ...
  app.post("/api/walk-forward", requireAuth, async (req, res) => {
    ...
    if (trainDays < 5 || trainDays > 200) {
      return res
        .status(400)
        .json({ error: "trainDays must be between 5 and 200" });
    }
    if (testDays < 3 || testDays > 60) {
      return res
        .status(400)
        .json({ error: "testDays must be between 3 and 60" });
    }
    if (totalWindows < 1 || totalWindows > 10) {
      return res
        .status(400)
        .json({ error: "totalWindows must be between 1 and 10" });
    }
    ...
    res.json({
      started: true,
      message: `Walk-forward evaluation started: ${totalWindows} windows, ${trainDays} train / ${testDays} test days each`,
    });
  });
  ...
  app.post("/api/simulations/reset", requireAuth, async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: err.message || "Failed to reset simulation data" });
    }
  });
  ...
  app.post("/api/simulations/archive", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: err.message || "Failed to export archive" });
    }
  });
  ...
  app.get("/api/simulations/core-metrics", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: err.message || "Failed to compute metrics" });
    }
  });
  ...
  app.get("/api/simulations/report", requireAuth, async (req, res) => {
    try {
      ...
      res.json({
        ...
        wins: o?.wins ?? 0,
        winRate:
          total > 0 ? Number((((o?.wins ?? 0) / total) * 100).toFixed(1)) : 0,
  ...
  wss.on("connection", (ws) => {
    clients.add(ws);
    ws.send(
      JSON.stringify({ type: "connected", data: { timestamp: Date.now() } }),
    );
  ...
  app.get("/api/analytics/daily-report", requireAuth, (req, res) => {
    const {
      ...
      getAllTrades,
      getTradeCount,
    } = require("./analytics/tradeStore");
    const {
      buildDailySummary,
      formatDailySummary,
    } = require("./analytics/tradeAnalytics");
  ...
  app.post("/api/internal/rs-phase-a", async (req, res) => {
    ...
    const aggregate = (results: any[]) => {
      ...
      const medianMFE =
        sortedMFE.length > 0 ? sortedMFE[Math.floor(sortedMFE.length / 2)] : 0;
      const sortedMAE = [...allMAEs].sort((a, b) => a - b);
      const medianMAE =
        sortedMAE.length > 0 ? sortedMAE[Math.floor(sortedMAE.length / 2)] : 0;
  ...
  app.post("/api/internal/rs-phase-b", async (req, res) => {
    ...
    const runVariantOnDates = async (
      config: Partial<RSConfig>,
      dates: string[],
    ) => {
      const results: any[] = [];
      for (const date of dates) {
  ...
  app.post("/api/internal/rs-validate", async (req, res) => {
    ...
    const bySymbol: Record<
      string,
      { trades: number; rs: number[]; wins: number }
    > = {};
    const byRegime: Record<
      string,
      { trades: number; totalR: number; wins: number }
    > = {};
    ...
    for (const day of results) {
      ...
      for (let i = 0; i < tradeCount; i++) {
        ...
        allRs.push(r);
        if (day.tradeMFEs && i < day.tradeMFEs.length)
          allMFEs.push(day.tradeMFEs[i]);
        if (day.tradeMAEs && i < day.tradeMAEs.length)
          allMAEs.push(day.tradeMAEs[i]);
        const ticker =
          day.tradeTickers && i < day.tradeTickers.length
            ? day.tradeTickers[i]
            : undefined;
        const regime =
          day.tradeRegimes && i < day.tradeRegimes.length
            ? day.tradeRegimes[i]
            : undefined;
        if (ticker) {
          if (!bySymbol[ticker])
            bySymbol[ticker] = { trades: 0, rs: [], wins: 0 };
        ...
        if (regime) {
          if (!byRegime[regime])
            byRegime[regime] = { trades: 0, totalR: 0, wins: 0 };
    ...
    const avgMFE =
      allMFEs.length > 0
        ? allMFEs.reduce((a, b) => a + b, 0) / allMFEs.length
        : 0;
    const avgMAE =
      allMAEs.length > 0
        ? allMAEs.reduce((a, b) => a + b, 0) / allMAEs.length
        : 0;
    const medianMFE =
      allMFEs.length > 0
        ? [...allMFEs].sort((a, b) => a - b)[Math.floor(allMFEs.length / 2)]
        : 0;
    ...
    for (const [sym, data] of Object.entries(bySymbol)) {
      const symAvgR =
        data.rs.length > 0
          ? data.rs.reduce((a, b) => a + b, 0) / data.rs.length
          : 0;
      symbolSummary[sym] = {
        trades: data.trades,
    ...
    for (const [reg, data] of Object.entries(byRegime)) {
      regimeSummary[reg] = {
        trades: data.trades,
        avgR: Number((data.totalR / data.trades).toFixed(3)),
        winRate: Number((data.wins / data.trades).toFixed(3)),
      };
    }
  ...
  app.post("/api/internal/gap-phase-a", async (req, res) => {
    ...
    const dates: string[] =
      userDates && Array.isArray(userDates) && userDates.length > 0
        ? userDates
        : defaultDates;
    const tickerList = tickers ?? [
      "AAPL",
    ...
    const aggregate = (results: any[]) => {
      ...
      for (const day of results) {
        ...
        trades += day.trades ?? 0;
        wins += day.wins ?? 0;
        totalR += (day.tradeRs as number[]).reduce(
        ...
        if (day.byTier) {
          for (const [tier, data] of Object.entries(
            day.byTier as Record<string, any>,
          )) {
            const dir = tier.includes("long") ? "LONG" : "SHORT";
            if (!byDirection[dir])
              byDirection[dir] = { trades: 0, totalR: 0, wins: 0 };
  ...
  app.post("/api/internal/orf-walkforward", async (req, res) => {
    ...
    if (
      !devDates ||
      !Array.isArray(devDates) ||
      !testDates ||
      !Array.isArray(testDates)
    ) {
      return res
        .status(400)
        .json({ error: "devDates and testDates arrays required" });
    }
    const tickerList = tickers ?? [
      "AAPL",
      "MSFT",
    ...
    function aggregateWindow(dayResults: any[]) {
      ...
      for (const day of dayResults) {
        ...
        for (let i = 0; i < tradeCount; i++) {
          ...
          if (day.tradeMAEs && i < day.tradeMAEs.length)
            allMAEs.push(day.tradeMAEs[i]);
          if (day.tradeHit1R && i < day.tradeHit1R.length)
            allHit1R.push(day.tradeHit1R[i]);
          if (day.tradeHitTarget && i < day.tradeHitTarget.length)
            allHitTarget.push(day.tradeHitTarget[i]);
          if (day.tradeSlippageCostsR && i < day.tradeSlippageCostsR.length)
            allSlipR.push(day.tradeSlippageCostsR[i]);
          if (
            day.tradeScratchAfterPartial &&
            i < day.tradeScratchAfterPartial.length
          )
            allScratch.push(day.tradeScratchAfterPartial[i]);
          ...
          if (ticker) {
            ...
            if (r > 0) bySymbol[ticker].wins++;
            else bySymbol[ticker].losses++;
          }
        }

        const dayAvgR =
          tradeCount > 0
            ? day.tradeRs.reduce((a: number, b: number) => a + b, 0) /
              tradeCount
            : 0;
        dailyPnls.push({
          date: day.date,
          trades: day.trades,
          netPnl: day.netPnl ?? 0,
          avgR: Number(dayAvgR.toFixed(3)),
        });

        const dow = new Date(day.date + "T12:00:00Z").toLocaleDateString(
          "en-US",
          { weekday: "short" },
        );
        if (!byDayOfWeek[dow])
          byDayOfWeek[dow] = { wins: 0, losses: 0, pnl: 0, trades: 0 };
      ...
      const hit1RPct =
        allHit1R.length > 0
          ? allHit1R.reduce((a, b) => a + b, 0) / allHit1R.length
          : 0;
      const hitTargetPct =
        allHitTarget.length > 0
          ? allHitTarget.reduce((a, b) => a + b, 0) / allHitTarget.length
          : 0;
      const avgSlipR =
        allSlipR.length > 0
          ? allSlipR.reduce((a, b) => a + b, 0) / allSlipR.length
          : 0;
      const scratchPct =
        allScratch.length > 0
          ? allScratch.reduce((a, b) => a + b, 0) / allScratch.length
          : 0;
      ...
      const avgLossR =
        losers.length > 0
          ...
          : 0;
      const profitFactor =
        losers.length > 0 && Math.abs(avgLossR * losers.length) > 0
          ? (avgWinR * winners.length) / Math.abs(avgLossR * losers.length)
          : 0;
      ...
      for (let i = 0; i < allRegimes.length; i++) {
        ...
        if (!regimeSummary[reg])
          regimeSummary[reg] = { trades: 0, wins: 0, totalR: 0 };
      ...
      for (const [sym, data] of Object.entries(bySymbol)) {
        ...
        symbolSummary[sym] = {
          ...
          avgR: Number(symAvgR.toFixed(3)),
          winRate:
            data.trades > 0 ? Number((data.wins / data.trades).toFixed(3)) : 0,
        };
      }
    ...
    try {
      ...
      const degradation =
        devAgg.core.avgR !== 0
          ? Number(
              (
                (testAgg.core.avgR - devAgg.core.avgR) /
                Math.abs(devAgg.core.avgR)
              ).toFixed(3),
            )
          : 0;
      ...
      res.json({
        ...
        verdict: testAgg.core.avgR >= -0.1 ? "MARGINAL" : "NO_EDGE",
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/internal/smallcap-validate", async (req, res) => {
    ...
    const aggregate = (results: any[]) => {
      ...
      for (const day of results) {
        ...
        if (day.tradeSlippageCostsR)
          allSlippageCostsR.push(...day.tradeSlippageCostsR);
      ...
      const p75MFE =
        sortedMFE.length > 0
          ? sortedMFE[Math.floor(sortedMFE.length * 0.75)]
          : 0;
      const avgMFE =
        allMFEs.length > 0
          ? allMFEs.reduce((a, b) => a + b, 0) / allMFEs.length
      ...
      const avgMAE =
        allMAEs.length > 0
          ...
          : 0;
      const avgSlippageR =
        allSlippageCostsR.length > 0
          ? allSlippageCostsR.reduce((a, b) => a + b, 0) /
            allSlippageCostsR.length
          : 0;

      const profitFactor =
        grossLossR > 0 ? grossWinR / grossLossR : grossWinR > 0 ? Infinity : 0;
      ...
      for (const [sym, data] of Object.entries(bySymbol)) {
        ...
        symbolSummary[sym] = {
          ...
          winRate: Number((data.wins / data.trades).toFixed(3)),
          maxR:
            data.rs.length > 0 ? Number(Math.max(...data.rs).toFixed(3)) : 0,
          minR:
            data.rs.length > 0 ? Number(Math.min(...data.rs).toFixed(3)) : 0,
        };
      }
      ...
      for (const q of qualRejected) {
        ...
        const bucket = reason.includes("gap")
          ? "gap_too_small"
          : reason.includes("dollar vol")
            ? "dollar_vol_low"
            : reason.includes("vol") && reason.includes("premarket")
              ? "premarket_vol_low"
              : reason.includes("vol")
                ? "avg_vol_low"
                : reason.includes("ATR")
                  ? "atr_too_low"
                  : reason.includes("float")
                    ? "float_too_big"
                    : reason.includes("price") || reason.includes("open")
                      ? "price_out_of_range"
                      : "other";
        rejectionReasons[bucket] = (rejectionReasons[bucket] ?? 0) + 1;
      }

      const avgPremarketVol =
        qualPassed.length > 0
          ? qualPassed.reduce(
              (s: number, q: any) => s + (q.premarketVolume ?? 0),
              0,
            ) / qualPassed.length
          : 0;
      const avgFloat =
        qualPassed.length > 0
          ? qualPassed.reduce(
              (s: number, q: any) => s + (q.floatShares ?? 0),
              0,
            ) / qualPassed.length
          : 0;
      const candidatesPerDay =
        dates.length > 0 ? qualPassed.length / dates.length : 0;
      ...
      return {
        core: {
          ...
          medianR:
            sortedRs.length > 0
              ? Number(sortedRs[Math.floor(sortedRs.length / 2)].toFixed(3))
              : 0,
          profitFactor: Number(profitFactor.toFixed(3)),
        ...
        qualification: {
          ...
          passRate:
            qualTotal > 0
              ? Number(((qualPassed.length / qualTotal) * 100).toFixed(1))
              : 0,
        ...
        ...(useDynamicScanner
          ? {
              dynamicScanner: {
                ...
                avgQualifiedPerDay:
                  dates.length > 0
                    ? Number((totalScannerQualified / dates.length).toFixed(1))
                    : 0,
                avgLongPerDay:
                  dates.length > 0
                    ? Number((totalScannerLong / dates.length).toFixed(1))
                    : 0,
    ...
    try {
      ...
      if (
        useDynamicScanner &&
        userGapScanConfig?.useFullMarket &&
        dates.length > 1
      ) {
        const { batchScanForGappers } = await import(
          "./strategy/batchGapScanner"
        );
        const gapCfg = { ...(userGapScanConfig ?? {}), useFullMarket: true };
        if (scConfig.minPrice)
          gapCfg.minPrice = gapCfg.minPrice ?? scConfig.minPrice;
        if (scConfig.maxPrice)
          gapCfg.maxPrice = gapCfg.maxPrice ?? scConfig.maxPrice;
        if (scConfig.minGapPct)
          gapCfg.minGapPct = gapCfg.minGapPct ?? scConfig.minGapPct;

        const batchResult = await batchScanForGappers(
          dates[0],
      ...
      res.json({
        ...
        config: {
          smallCap: { ...DEFAULT_SMALLCAP_CONFIG, ...scConfig },
          pullback: { ...DEFAULT_PULLBACK_CONFIG, ...pbConfig },
        },
        dateRange: {
          start: dates[0],
        ...
        tickerCount: useDynamicScanner
          ? ((agg as any).dynamicScanner?.avgLongPerDay ?? "dynamic")
          : tickerList.length,
        ...agg,
        verdict,
        perDay: results.map((r) => ({
          ...
          losses: r.losses ?? 0,
          avgR:
            r.tradeRs && r.tradeRs.length > 0
              ? Number(
                  (
                    r.tradeRs.reduce((a: number, b: number) => a + b, 0) /
                    r.tradeRs.length
                  ).toFixed(3),
                )
              : null,
  ...
  app.post("/api/internal/smallcap-walkforward", async (req, res) => {
    ...
    const aggregateResults = (results: any[]) => {
      ...
      const sortedRs = [...allRs].sort((a, b) => a - b);
      const medianR =
        sortedRs.length > 0 ? sortedRs[Math.floor(sortedRs.length / 2)] : 0;
      const sortedMFE = [...allMFEs].sort((a, b) => a - b);
      const medianMFE =
        sortedMFE.length > 0 ? sortedMFE[Math.floor(sortedMFE.length / 2)] : 0;
      const avgMFE =
    ...
    try {
      ...
      const gapCfg = { ...(userGapScanConfig ?? {}), useFullMarket: true };
      if (baseScConfig.minPrice)
        gapCfg.minPrice = gapCfg.minPrice ?? baseScConfig.minPrice;
      if (baseScConfig.maxPrice)
        gapCfg.maxPrice = gapCfg.maxPrice ?? baseScConfig.maxPrice;
      ...
      const runWindowForCombo = async (
        ...
        const scConfig = {
          ...
          trailActivationR:
            combo.trailOffsetR < 1.0
              ? combo.trailOffsetR + 0.5
              : combo.trailOffsetR + 0.25,
        };
      ...
      const devResults: Array<{
        combo: (typeof combos)[0];
        metrics: ReturnType<typeof aggregateResults>;
      }> = [];

      for (const combo of combos) {
        const metrics = await runWindowForCombo(
          combo,
          devDates,
          batchGapResults,
        );
      ...
      const testMetrics = await runWindowForCombo(
        bestDev.combo,
        testDates,
        batchGapResults,
      );
      ...
      const verdict =
        passCount === 4 ? "PASS" : passCount >= 3 ? "MARGINAL_PASS" : "FAIL";
      ...
      res.json({
        ...
        devWindow: {
          ...
          days: devDates.length,
        },
        testWindow: {
          start: testStartDate,
          end: testEndDate,
          days: testDates.length,
        },
        parameterGrid: {
          gapThresholds: gapThresholds.map(
            (g: number) => `${(g * 100).toFixed(0)}%`,
          ),
          premarketVolFloors: premarketVolFloors.map(
  ...
  app.post("/api/internal/volatility-cluster-test", async (req, res) => {
    ...
    const { startDate, endDate, clusterConfig: userClusterConfig } = req.body;
    ...
    try {
      const { batchComputeClusterActivation } = await import(
        "./strategy/volatilityClusterFilter"
      );
      const { batchScanForGappers } = await import(
        "./strategy/batchGapScanner"
      );
      ...
      const gapCfg = {
        useFullMarket: true,
        minPrice: frozenConfig.minPrice,
        maxPrice: frozenConfig.maxPrice,
          ...
          if (dayGap) {
            tickersForDay = dayGap.qualifiers
              .filter(
                (q: any) =>
                  q.gapDirection === "LONG" &&
                  Math.abs(q.gapPct) >= frozenConfig.minGapPct,
              )
              .map((q: any) => q.ticker);
          }
      ...
      const calculateStats = (logs: any[]) => {
        ...
        const mfe2RPct =
          trades > 0
            ? (allDetails.filter((t) => t.mfe >= 2.0).length / trades) * 100
            : 0;
  ...
  app.post("/api/internal/smallcap-extended-oos", async (req, res) => {
    const userId = getSharedUserId() ?? "internal";
    const { startDate, endDate, gapScanConfig: userGapScanConfig } = req.body;
    ...
    try {
      ...
      const gapCfg = { ...(userGapScanConfig ?? {}), useFullMarket: true };
      if (frozenConfig.minPrice)
        gapCfg.minPrice = gapCfg.minPrice ?? frozenConfig.minPrice;
      if (frozenConfig.maxPrice)
        gapCfg.maxPrice = gapCfg.maxPrice ?? frozenConfig.maxPrice;
      ...
      const daysWithTrades = dayResults.filter(
        (d) => (d.trades ?? 0) > 0,
      ).length;
      const spreadRejects = dayResults.reduce(
        (a, d) => a + (d.spreadRejects ?? 0),
        0,
      );
      ...
      const top3DaysR = [...dailyRValues]
        .sort((a, b) => b - a)
        .slice(0, 3)
        .reduce((a, b) => a + b, 0);
      const pctFromTop3Days = totalR !== 0 ? (top3DaysR / totalR) * 100 : 0;

      const gapSizeDist: Record<
        string,
        { trades: number; avgR: number; winRate: number; totalR: number }
      > = {
      ...
      const gapBucketWins: Record<string, number> = {
        "6-8%": 0,
        "8-10%": 0,
        "10-15%": 0,
        "15-20%": 0,
      ...
      for (const key of Object.keys(gapSizeDist)) {
        ...
        b.winRate =
          b.trades > 0
            ? Number(((gapBucketWins[key] / b.trades) * 100).toFixed(1))
            : 0;
      ...
      const passCriteria = {
        ...
        no_regime_collapse:
          worstDay > -5 &&
          tail3RCount <= Math.max(1, Math.floor(trades * 0.03)),
      };
      const passCount = Object.values(passCriteria).filter(Boolean).length;
      const verdict =
        passCount === 4
      ...
      res.json({
        ...
        coreMetrics: {
          ...
          mfeAvg: Number(avgMFE.toFixed(3)),
          mfe1RPct:
            trades > 0 ? Number(((mfe1R / trades) * 100).toFixed(1)) : 0,
          mfe2RPct:
            trades > 0 ? Number(((mfe2R / trades) * 100).toFixed(1)) : 0,
          maeMedian: Number(medianMAE.toFixed(3)),
          worstTradeR: Number(worstTradeR.toFixed(3)),
          tail3RCount,
          spreadRejects,
